...
def jac_of_constraint(x, *args):
    ...
    if args[0] == 'model':
        ...
        #phiの変化率に対する不等式制約    
            ...
            jac_cons[3][i] = 2 * (phi[i+1] - phi[i])
            jac_cons[3][i+1] = -2 * (phi[i+1] - phi[i])
            
        #vの変化率に対する不等式制約    
        elif args[1][0] == 'v':
            jac_cons[4][i] = 2 * (v[i+1] - v[i])
            jac_cons[4][i+1] = -2 * (v[i+1] - v[i])
...
def constraint(x, *args):
    ...
    if args[0] == 'model':
        ...
        #phiの変化率に対する不等式制約
            ...
            value = p.a_phi * p.dt ** 2 - (phi[i+1] - phi[i]) ** 2
            
        #vの変化率に対する不等式制約
        elif args[1][0] == 'v':
            value = p.a_v * p.dt ** 2 - (v[i+1] - v[i]) ** 2
